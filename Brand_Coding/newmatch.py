...
使用方法：
1、修改文件路径
2、根据所处理的产品类别，使用匹配该类别的部分代码，category_to_process替换为该类别的名称
3、在test_path中查看匹配结果
"""


# 加载数据：修改sheetname为你要处理的产品类别（如skincare、hair等）
def load_data(target_type_path, raw_data_path, sheet_name):
    # 替换为要进行coding的某类别产品文件路径（格式：第一列为某类别产品反馈；第二列为某类别产品的品牌标准名称/用户回答），也可修改代码，改为指定反馈表格中的某行某列
    target_type_df = pd.read_excel(target_type_path, sheet_name=sheet_name)
    # 替换为brandlist表格路径
    raw_data_df = pd.read_excel(raw_data_path, sheet_name=sheet_name)
    return target_type_df, raw_data_df

# 输出的匹配结果路径（避免直接修改原始brandlist，暂存中间文件）
def save_data(df, test_path):
...
"""
def match_skincare_makeup(target_type_df, raw_data_df):
    # 遍历数据
...
"""
def match_perfume(target_type_df, raw_data_df):
    # 遍历数据
    for index, row in target_type_df.iterrows():
        first_col_value = row[0]
        second_col_value = row[1]
...
def process_hair_table(target_type_df, raw_data_df):
    # 遍历数据
    for index, row in target_type_df.iterrows():
        first_col_values = []
        second_col_value = row[1]
...
men列表特殊处理
"""
def process_men_table(target_type_df, raw_data_df):
...
def select(category):
    ...
    # 加载数据
    target_type_df, raw_data_df = load_data(target_type_path, raw_data_path, sheet_name=category)

    # 根据类别执行匹配逻辑
    if category == 'skincare' or category == 'makeup':
        match_skincare_makeup(target_type_df, raw_data_df)

    elif category == 'perfume':
        match_perfume(target_type_df, raw_data_df)

    elif category == 'hair':
        process_hair_table(target_type_df, raw_data_df)

    elif category == 'men':
        process_men_table(target_type_df, raw_data_df)

    # 保存匹配结果
    save_data(raw_data_df, test_path)

if __name__ == "__main__":
    # 用户指定要处理的产品类别
    category_to_process = 'men'  # 可以是 'skincare', 'makeup', 'perfume', 'hair', 或者 'men'
    select(category_to_process)
